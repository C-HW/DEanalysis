...
```{r eval=FALSE}
pdf("~/Google Drive/My Drive/spatial/10X/graph_panels/panel4/raw_library.pdf", width = 4, height = 3)
p_countbygroup
dev.off()
```
...
```
```{r eval=FALSE}
pdf("~/Google Drive/My Drive/spatial/10X/graph_panels/panel4/ttest_meandiff_logUMI_logCPM.pdf", width = 12, height = 3)
ggarrange(p_ttest_raw_cpm, p_ttest_raw_vst, p_ttest_raw_integrated, ncol = 3)
dev.off()
```
...
geom_histogram(fill="white", alpha=0.5, position="identity") + theme_bw() + theme(panel.grid = element_blank()) + xlab(NULL) + ylab("Count") + ylim(c(0,3000)) + ggtitle(titlelist[i]) + scale_colour_discrete(guide = guide_legend(title = NULL),labels = c("p-value", "adj.p-value"))
}
figure = ggarrange(plotlist = p, common.legend = TRUE, legend="bottom", nrow = 2, ncol = 4)
...
```{r eval=FALSE}
pdf("~/Google Drive/My Drive/spatial/10X/graph_panels/panel3/heatmap_VST.pdf", width = 5, height = 5)
pheatmap(mat[,rownames(annotation_df)], 
         main = "VST data",
         annotation_col = annotation_df, 
         annotation_colors = annotation_colors,
...
```
```{r eval=FALSE}
pdf("~/Google Drive/My Drive/spatial/10X/graph_panels/panel4/heatmap_integrated.pdf", width = 5, height = 5)
pheatmap(mat[,rownames(annotation_df)], 
         main = paste0("Poisson-glmm DEGs\nin VST data (", length(hits_in_int)," DEGs)"),
         annotation_col = annotation_df, 
         annotation_row = annotation_log2FC,
         annotation_colors = annotation_colors,
         cluster_rows=F, cluster_cols=F, 
