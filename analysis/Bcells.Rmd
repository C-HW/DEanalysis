...
geom_histogram(fill="white", alpha=0.5, position="identity") + theme_bw() + theme(panel.grid = element_blank()) + xlab(NULL) + ylab("Count") + ylim(c(0,1100)) + ggtitle(titlelist[i]) + scale_colour_discrete(guide = guide_legend(title = NULL),labels = c("p-value", "adj.p-value"))
}
figure = ggarrange(plotlist = p, common.legend = TRUE, legend="bottom", nrow = 2, ncol = 4)
...
```{r eval=FALSE}
pdf("~/Google Drive/My Drive/spatial/10X/graph_panels/panel7/positive_dotplot.pdf", width = 6, height = 5)
dotplot(go_enrich)
pdf("~/Google Drive/My Drive/spatial/10X/graph_panels/panel7/positive_cnetplot.pdf", width = 12, height = 6)
cnetplot(go_enrich, categorySize="pvalue", foldChange=gene_list)
dev.off()
```
### Negative DEGs
...
# sort the list in decreasing order (required for clusterProfiler)
gene_list = sort(gene_list, decreasing = TRUE)
negative_genes = which(Kang_pois_glmm_df[[key]]$hits) & Kang_pois_glmm_df[[key]]$log2FC < 0
genes = Kang_pois_glmm_df[[key]]$genes[negative_genes]
# create GO object
go_enrich = enrichGO(gene = genes,
                      universe = names(gene_list),
                      OrgDb = "org.Hs.eg.db", 
...
```{r eval=FALSE}
pdf("~/Google Drive/My Drive/spatial/10X/graph_panels/panel7/negative_dotplot.pdf", width = 6, height = 5)
dotplot(go_enrich)
pdf("~/Google Drive/My Drive/spatial/10X/graph_panels/panel7/negative_cnetplot.pdf", width = 12, height = 6)
cnetplot(go_enrich, categorySize="pvalue", foldChange=gene_list)
dev.off()
```
...
# sort the list in decreasing order (required for clusterProfiler)
gene_list = sort(gene_list, decreasing = TRUE)
negative_genes = which(Kang_DESeq2.muscat[[key]]$hits & Kang_DESeq2.muscat[[key]]$log2FC < 0)
genes = Kang_DESeq2.muscat[[key]]$genes[negative_genes]
# create GO object
go_enrich = enrichGO(gene = genes,
                      universe = names(gene_list),
                      OrgDb = "org.Hs.eg.db", 
...
# sort the list in decreasing order (required for clusterProfiler)
gene_list = sort(gene_list, decreasing = TRUE)
negative_genes = which(Kang_MMpoisson[[key]]$hits & Kang_MMpoisson[[key]]$log2FC < 0)
genes = Kang_MMpoisson[[key]]$genes[negative_genes]
# create GO object
go_enrich = enrichGO(gene = genes,
                      universe = names(gene_list),
                      OrgDb = "org.Hs.eg.db", 
