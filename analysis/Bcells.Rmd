...
Kang_Wilcox_Seurat_df[[cur_key]]$genes = gsub("-", "_", rownames(Kang_Wilcox_Seurat_df[[cur_key]]))
for(i in 1:length(Kang_Wilcox_Seurat_df[[cur_key]]$genes)){
  if(Kang_Wilcox_Seurat_df[[cur_key]]$genes[i] %in%rownames(subgroupsce)){}else{
    Kang_Wilcox_Seurat_df[[cur_key]]$genes[i] = gsub("_ENSG", "-ENSG", Kang_Wilcox_Seurat_df[[cur_key]]$genes[i])
  }
}
```
...
geom_histogram(fill="white", alpha=0.5, position="identity") + theme_bw() + theme(panel.grid = element_blank()) + xlab(NULL) + ylab("Count") + coord_cartesian(c(0,1100)) + ggtitle(titlelist[i]) + scale_colour_discrete(guide = guide_legend(title = NULL),labels = c("p-value", "adj.p-value"))
}
figure = ggarrange(plotlist = p, common.legend = TRUE, legend="bottom", nrow = 2, ncol = 4)
...
## Poisson-glmm DEGs
```{r}
sort_log2FC = sort(abs(Kang_pois_glmm_df[[cur_key]]$log2FC[Kang_pois_glmm_df[[cur_key]]$hits]), index.return = T, decreasing = T)
...
#row annotation
annotation_log2FC = data.frame(abs_log2FC = abs(Kang_pois_glmm_df[[cur_key]]$log2FC[match(hitgenes,Kang_pois_glmm_df[[cur_key]]$genes)]))
...
### pb-DESeq2
```{r}
sort_log2FC = sort(abs(Kang_DESeq2.muscat[[cur_key]]$log2FC[Kang_DESeq2.muscat[[cur_key]]$hits]), index.return = T, decreasing = T)
...
### MMpoisson
```{r}
sort_log2FC = sort(abs(dflist[[8]][[cur_key]]$log2FC[dflist[[8]][[cur_key]]$hits]), index.return = T, decreasing = T)
