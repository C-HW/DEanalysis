...
load("./data/10X_DEresult_update2.RData")
...
zeroprop_df = rbind(data.frame(zero_prop = rowMeans(assays(sce_fallopian_tubes)$counts[, sce_fallopian_tubes$hippo_cluster %in% group1]== 0), genemean = rowMeans(assays(sce_fallopian_tubes)$counts[, sce_fallopian_tubes$hippo_cluster %in% group1]), Group = paste(group1, collapse = "_")),
                data.frame(zero_prop = rowMeans(assays(sce_fallopian_tubes)$counts[, sce_fallopian_tubes$hippo_cluster %in% group2]== 0), genemean = rowMeans(assays(sce_fallopian_tubes)$counts[, sce_fallopian_tubes$hippo_cluster %in% group2]), Group = paste(group2, collapse = "_")),
                data.frame(zero_prop = rowMeans(assays(sce_fallopian_tubes)$counts[, sce_fallopian_tubes$hippo_cluster %in% c(group1,group2)]== 0), genemean = rowMeans(assays(sce_fallopian_tubes)$counts[, sce_fallopian_tubes$hippo_cluster %in% c(group1,group2)]), Group = cur_key)
                )
...
```{r}
ks_result = ks.test(subset(count_cell_df, Group == "2_19", totalcount)$totalcount, subset(count_cell_df, Group == "8_17", totalcount)$totalcount)
t_test = t.test(subset(count_cell_df, Group == "2_19", totalcount)$totalcount, subset(count_cell_df, Group == "8_17", totalcount)$totalcount)
print(ks_result)
print(t_test)
```
...
integrated_mean_DE = simple_mean_DE(subgroup_Seurat@assays$integrated$data, cellgroup == 1, cellgroup == 2)
...
### UMI counts
```{r}
sort_log2FC = sort(abs(pois_glmm_df[[cur_key]]$log2FC[pois_glmm_df[[cur_key]]$hits]), index.return = T, decreasing = T)
...
#row annotation
annotation_log2FC = data.frame(log2FC = abs(pois_glmm_df[[cur_key]]$log2FC[match(c(hits_in_int, hits_notin_int) ,pois_glmm_df[[cur_key]]$genes)]), `Integrated data` = c(rep("Incluede", length(hits_in_int)), rep("Not included", length(hits_notin_int))))
...
### pb-DESeq2
```{r}
sort_log2FC = sort(abs(DESeq2.muscat[[cur_key]]$log2FC[DESeq2.muscat[[cur_key]]$hits]), index.return = T, decreasing = T)
...
### pb-edgeR
```{r}
sort_log2FC = sort(abs(edgeR.cpm[[cur_key]]$log2FC[edgeR.cpm[[cur_key]]$hits]), index.return = T, decreasing = T)
hitgenes_edgeR = edgeR.cpm[[cur_key]]$genes[which(edgeR.cpm[[cur_key]]$hits)][sort_log2FC$ix]
...
### Binomial-glmm
```{r}
sort_log2FC = sort(abs(binom_glmm_df[[cur_key]]$log2FC[binom_glmm_df[[cur_key]]$hits]), index.return = T, decreasing = T)
...
### Wilcox
```{r}
sort_log2FC = sort(abs(Wilcox_Seurat_integrated_df[[cur_key]]$avg_log2FC[Wilcox_Seurat_integrated_df[[cur_key]]$hits]), index.return = T, decreasing = T)
...
### MAST
```{r}
sort_log2FC = sort(abs(MAST_donor_df[[cur_key]]$log2FC[MAST_donor_df[[cur_key]]$hits]), index.return = T, decreasing = T)
hitgenes_MAST = MAST_donor_df[[cur_key]]$genes[which(MAST_donor_df[[cur_key]]$hits)][sort_log2FC$ix]
...
### MMvst
```{r}
sort_log2FC = sort(abs(MMvst[[cur_key]]$log2FC[MMvst[[cur_key]]$hits]), index.return = T, decreasing = T)
hitgenes_MMvst = MMvst[[cur_key]]$genes[which(MMvst[[cur_key]]$hits)][sort_log2FC$ix]
...
### MMpoisson
```{r}
sort_log2FC = sort(abs(MMpoisson[[cur_key]]$log2FC[MMpoisson[[cur_key]]$hits]), index.return = T, decreasing = T)
