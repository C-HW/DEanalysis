...
library(pathview)
assign_subgroup(c(8,17), c(2,19))
pois_glmm_df[[key]]$log2FC = -pois_glmm_df[[key]]$log2FC
binom_glmm_df[[key]]$log2FC = -binom_glmm_df[[key]]$log2FC
MAST_donor_df[[key]]$log2FC = -MAST_donor_df[[key]]$log2FC
Wilcox_Seurat_df[[key]]$genes = rownames(Wilcox_Seurat_df[[key]])
```
```{r}
...
```{r}
df = data.frame(Group = colData(subgroupsce)$hippo_cluster, Donor = colData(subgroupsce)$donor, Count = 1)
...
count_cell_df = data.frame(totalcount = apply(subgroupsce@assays@data$counts, 2, sum), Group = sapply(cellgroup, FUN = function(x)ifelse(x == 1, paste(group1, collapse = "_"), paste(group2, collapse = "_"))), Donor = colData(subgroupsce)$donor)
...
p_countbygroup = ggplot(count_cell_df, aes(x = totalcount, color = Group)) +
  geom_histogram(aes(y=..density..), fill= "white", alpha=0.5, position="identity", bin = 40) +
...
zeroprop_df = rbind(data.frame(zero_prop = rowMeans(inputData@assays@data$counts[, inputData$hippo_cluster %in% group1]== 0), genemean = rowMeans(inputData@assays@data$counts[, inputData$hippo_cluster %in% group1]), Group = paste(group1, collapse = "_")),
                data.frame(zero_prop = rowMeans(inputData@assays@data$counts[, inputData$hippo_cluster %in% group2]== 0), genemean = rowMeans(inputData@assays@data$counts[, inputData$hippo_cluster %in% group2]), Group = paste(group2, collapse = "_")),
                data.frame(zero_prop = rowMeans(inputData@assays@data$counts[, inputData$hippo_cluster %in% c(group1,group2)]== 0), genemean = rowMeans(inputData@assays@data$counts[, inputData$hippo_cluster %in% c(group1,group2)]), Group = key)
                )
...
p_zeroprop = ggplot(zeroprop_df, aes(x = genemean, y = zero_prop, Group = Group)) + 
  ...
  scale_color_manual(name = "Group", values = c(combined_palette,"darkmagenta")) + 
...
pdf("~/Google Drive/My Drive/spatial/10X/graph_panels/panel6/data_summary.pdf", width = 14, height = 3)
...
```{r}
subset_ind = colData(inputData)$hippo_cluster%in% c(group1,group2)
...
cpm_mean_DE = simple_mean_DE(log2(assays(inputData_cpm[,subset_ind])$counts+1), cellgroup == 1, cellgroup == 2)
```
```{r, fig.width=12, fig.height=4}
p_ttest_raw_integrated = ggplot(data = data.frame(raw_t = raw_mean_DE[intersect(rownames(raw_mean_DE), rownames(integrated_mean_DE)), "t"], int_t = integrated_mean_DE[intersect(rownames(raw_mean_DE), rownames(integrated_mean_DE)), "t"]), aes(x = int_t, y = raw_t)) + 
  geom_point(size = 0.5, alpha = 0.3, color = "#7C81AD") +
  geom_abline(slope = 1, intercept = 0, linetype = 2) +
  ylim(c(0,15)) +
  xlab("t score of mean difference test (Integrated data)") + 
  ylab("t score of mean difference test (Log2 Raw UMI)") + 
  theme_bw() + 
  theme(panel.grid = element_blank()) +
  xlim(c(0,15))

p_ttest_raw_vst = ggplot(data = data.frame(raw_t = raw_mean_DE[intersect(rownames(raw_mean_DE), rownames(vst_mean_DE)), "t"], vst_t = vst_mean_DE[intersect(rownames(raw_mean_DE), rownames(vst_mean_DE)), "t"]), aes(x = vst_t, y = raw_t)) + 
  geom_point(size = 0.5, alpha = 0.3, color = "#7C81AD") +
  geom_abline(slope = 1, intercept = 0, linetype = 2) +
  ylim(c(0,15)) +
  xlab("t score of mean difference test (VST data)") + 
  ylab("t score of mean difference test (Log2 Raw UMI)") + 
  theme_bw() + 
  theme(panel.grid = element_blank()) +
  xlim(c(0,15))

p_ttest_raw_cpm = ggplot(data = data.frame(raw_t = raw_mean_DE[intersect(rownames(raw_mean_DE), rownames(cpm_mean_DE)), "t"], cpm_t = cpm_mean_DE[intersect(rownames(raw_mean_DE), rownames(cpm_mean_DE)), "t"]), aes(x = cpm_t, y = raw_t)) + 
  geom_point(size = 0.5, alpha = 0.3, color = "#7C81AD") +
  geom_abline(slope = 1, intercept = 0, linetype = 2) +
  ylim(c(0,15)) +
  xlab("t score of mean difference test (Log2 CPM data)") + 
  ylab("t score of mean difference test (Log2 Raw UMI)") + 
  theme_bw() + 
  theme(panel.grid = element_blank()) +
  xlim(c(0,15))
...
pdf("~/Google Drive/My Drive/spatial/10X/graph_panels/panel6/input_hits.pdf", width = 8, height = 8)
...
geom_histogram(fill="white", alpha=0.5, position="identity") + theme_bw() + theme(panel.grid = element_blank()) + xlab(NULL) + ylab("Count") + ylim(c(0,2000)) + ggtitle(titlelist[i]) + scale_colour_discrete(guide = guide_legend(title = NULL),labels = c("p-value", "adj.p-value"))
}
figure = ggarrange(plotlist = p, common.legend = TRUE, legend="bottom", nrow = 2, ncol = 4)
...
for (i in 1:length(dflist)){
  ...
  for (j in 1:2){
    ...
    df = rbind(df, data.frame(Log2mean = log2(fun(inputData@assays@data$counts[hits_gene,cellind])), Group = clustername[j], method = titlelist[i]))
  }
}
...
```{r, eval=FALSE}
pdf("~/Google Drive/My Drive/spatial/10X/graph_panels/panel6/violin_log2mean.pdf", width = 8, height = 8)
ggplot(df, aes(x = factor(method,levels = rev(titlelist)), y=Log2mean, fill = factor(Group, levels = rev(levels(Group))))) +
  ...
  ylim(c(-3,4)) + 
  xlab("") + 
  scale_fill_manual(values = combined_palette, breaks = levels(df$Group)) +
  labs(fill = "Group") +
  coord_flip()
dev.off()
...
for (i in 1:length(dflist)){
  ...
  for (j in 1:3){
    cellind = inputData$hippo_cluster %in% clusterlist[[j]]
    df = rbind(df, data.frame(freq = fun(inputData@assays@data$counts[hits_gene,cellind]>0), cluster = clustername[j], method = titlelist[i]))
  }
}
...
pdf("~/Google Drive/My Drive/spatial/10X/graph_panels/panel6/violin_freq.pdf", width = 8, height = 8)
ggplot(df, aes(x = factor(method,levels = rev(titlelist)), y=freq, fill = factor(cluster, levels =  rev(levels(cluster))))) + 
  ...
  ylim(c(0,1)) + 
  xlab("") + 
  scale_fill_manual(values = c("gray", combined_palette), breaks = levels(df$cluster)) +
  labs(fill = "Group") +
  theme(axis.text.x = element_text(angle = 20, margin = margin(t = 15))) + coord_flip()
...
mat = subgroupsce@assays@data$counts[c(hits_in_int, hits_notin_int),]
#row annotation
...
```
```{r eval=FALSE}
pdf("~/Google Drive/My Drive/spatial/10X/graph_panels/panel6/heatmap_raw.pdf", width = 5, height = 5)
pheatmap(mat[,rownames(annotation_df)], 
         main = paste0("Poisson-glmm DEGs\nin UMI counts (", nrow(mat)," DEGs)"),
         annotation_col = annotation_df, 
         annotation_row = annotation_log2FC,
...
```{r eval=FALSE}
pdf("~/Google Drive/My Drive/spatial/10X/graph_panels/panel6/heatmap_VST.pdf", width = 5, height = 5)
pheatmap(mat[,rownames(annotation_df)], 
         main = "VST data",
         annotation_col = annotation_df, 
         annotation_colors = annotation_colors,
...
### CPM data
```{r}
mat = inputData_cpm@assays@data$counts[c(hits_in_int, hits_notin_int), subset_ind]/1000
...
### Integrated data
```{r}
mat = subgroupsce_integrated@assays@data$counts[hits_in_int,]
...
hits_notin_glmm = na.omit(hitgenes_deseq2[!hitgenes_deseq2%in%hitgenes][1:200])
...
```
```{r eval=FALSE}
pdf("~/Google Drive/My Drive/spatial/10X/graph_panels/panel6/heatmap_deseq2.pdf", width = 5, height = 5)
pheatmap(mat[,rownames(annotation_df)], 
         main = paste0("pb-DESeq2 DEGs \nnot identified by Poisson-glmm (",nrow(mat)," DEGs)"),
         annotation_col = annotation_df, 
         # annotation_row = annotation_log2FC,
...
hits_notin_glmm = na.omit(hitgenes_binom[!hitgenes_binom%in%hitgenes][1:200])
mat = subgroupsce@assays@data$counts[hits_notin_glmm,]
...
```
```{r eval=FALSE}
pdf("~/Google Drive/My Drive/spatial/10X/graph_panels/panel6/heatmap_binomial_glmm.pdf", width = 5, height = 5)
pheatmap(mat[,rownames(annotation_df)], 
         main = paste0("Binomial-glmm DEGs \nnot identified by Poisson-glmm (",nrow(mat)," DEGs)"),
         annotation_col = annotation_df, 
         annotation_colors = annotation_colors,
...
hits_notin_glmm = na.omit(hitgenes_MAST[!hitgenes_MAST%in%hitgenes][1:200])
mat = subgroupsce@assays@data$counts[hits_notin_glmm,]
...
```{r eval=FALSE}
pdf("~/Google Drive/My Drive/spatial/10X/graph_panels/panel6/heatmap_MAST_donor.pdf", width = 5, height = 5)
pheatmap(mat[,rownames(annotation_df)], 
         main = paste0("MAST DEGs \nnot identified by Poisson-glmm (",nrow(mat)," DEGs)"),
         annotation_col = annotation_df, 
         annotation_colors = annotation_colors,
...
hits_notin_glmm = na.omit(hitgenes_MMpoisson[!hitgenes_MMpoisson%in%hitgenes][1:200])
mat = subgroupsce@assays@data$counts[hits_notin_glmm,]
...
```{r eval=FALSE}
pdf("~/Google Drive/My Drive/spatial/10X/graph_panels/panel6/heatmap_MMpoisson.pdf", width = 5, height = 5)
pheatmap(mat[,rownames(annotation_df)], 
         main = paste0("MMpoisson DEGs \nnot identified by Poisson-glmm (",nrow(mat)," DEGs)"),
         annotation_col = annotation_df, 
         annotation_colors = annotation_colors,
...
hits_notin_MMpoisson = na.omit(hitgenes[!hitgenes%in%hitgenes_MMpoisson][1:200])
mat = subgroupsce@assays@data$counts[hits_notin_MMpoisson,]
...
pdf("~/Google Drive/My Drive/spatial/10X/graph_panels/panel6/heatmap_Extra_hits.pdf", width = 5, height = 5)
pheatmap(mat[,rownames(annotation_df)], 
         main = paste0("Poisson-glmm DEGs \nnot identified by MMpoisson (",nrow(mat)," DEGs)"),
         annotation_col = annotation_df, 
         annotation_colors = annotation_colors,
...
```{r eval=FALSE}
pdf("~/Google Drive/My Drive/spatial/10X/graph_panels/panel6/dotplot.pdf", width = 6, height = 5)
dotplot(go_enrich)
pdf("~/Google Drive/My Drive/spatial/10X/graph_panels/panel6/cnetplot.pdf", width = 12, height = 6)
cnetplot(go_enrich, categorySize="pvalue", foldChange=gene_list)
dev.off()
```
...
```{r eval=FALSE}
pdf("~/Google Drive/My Drive/spatial/10X/graph_panels/panel6/dotplot2.pdf", width = 6, height = 5)
dotplot(kk, 
        showCategory = 10, 
        title = "Enriched Pathways",
        font.size = 8)
...
```{r, fig.width = 10}
# tiff(file=paste0("result/",key,"/enrichment/kegg_cnetplot.tiff"), width = 1500, height = 700, res = 100)
