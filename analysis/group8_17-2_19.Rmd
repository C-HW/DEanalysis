...
pdf("../submission preparation/graph_panels/panel6/data_summary.pdf", width = 14, height = 3)
...
geom_histogram(fill="white", alpha=0.5, position="identity") + theme_bw() + theme(panel.grid = element_blank()) + xlab(NULL) + ylab("Count") + coord_cartesian(ylim = c(0, 3000)) + ggtitle(titlelist[i]) + scale_colour_discrete(guide = guide_legend(title = NULL),labels = c("p-value", "adj.p-value"))
}
figure = ggarrange(plotlist = p, common.legend = TRUE, legend="bottom", nrow = 2, ncol = 4)
...
ggplot(df, aes(x = factor(method,levels = rev(titlelist)), y=Log2mean, fill = factor(Group, levels = rev(levels(Group))))) +
  ...
  theme(axis.text.x = element_text(angle = 20, margin = margin(t = 15))) +
  labs(fill = "Group") +
  scale_fill_manual(values = combined_palette, breaks = levels(df$Group)) + coord_flip()
```
...
```
```{r eval=FALSE}
pdf("../submission preparation/graph_panels/panel6/heatmap_raw.pdf", width = 5, height = 5)
pheatmap(mat[,rownames(annotation_df)], 
         main = paste0("Poisson-glmm DEGs\nin UMI counts (", nrow(mat)," DEGs)"),
         annotation_col = annotation_df, 
         annotation_row = annotation_log2FC,
...
```{r eval=FALSE}
pdf("../submission preparation/graph_panels/panel6/heatmap_VST.pdf", width = 5, height = 5)
pheatmap(mat[,rownames(annotation_df)], 
         main = "VST data",
         annotation_col = annotation_df, 
         annotation_colors = annotation_colors,
...
```
```{r eval=FALSE}
png("../submission preparation/graph_panels/panel6/heatmap_deseq2.png", width = 500, height = 500)
pheatmap(mat[,rownames(annotation_df)], 
         main = paste0("pb-DESeq2 DEGs \nnot identified by Poisson-glmm (",nrow(mat)," DEGs)"),
         annotation_col = annotation_df, 
         # annotation_row = annotation_log2FC,
...
### pb-edgeR
```{r}
sort_log2FC = sort(abs(edgeR.cpm[[key]]$log2FC[edgeR.cpm[[key]]$hits]), index.return = T, decreasing = T)
hitgenes_edgeR = edgeR.cpm[[key]]$genes[which(edgeR.cpm[[key]]$hits)][sort_log2FC$ix]
hits_notin_glmm = na.omit(hitgenes_edgeR[!hitgenes_edgeR%in%hitgenes])
mat = assays(subgroupsce)$counts[hits_notin_glmm,]

# cell level
pheatmap(mat[,rownames(annotation_df)], 
         main = paste0("pb-edgeR DEGs \nnot identified by Poisson-glmm (",nrow(mat)," DEGs)"),
         annotation_col = annotation_df, 
         # annotation_row = annotation_log2FC,
         annotation_colors = annotation_colors,
         cluster_rows=F, cluster_cols=F, 
...
```{r eval=FALSE}
png("../submission preparation/graph_panels/panel6/heatmap_edgeR.png", width = 500, height = 500)
pheatmap(mat[,rownames(annotation_df)], 
         main = paste0("pb-edgeR DEGs \nnot identified by Poisson-glmm (",nrow(mat)," DEGs)"),
         annotation_col = annotation_df, 
         # annotation_row = annotation_log2FC,
...
```
```{r eval=FALSE}
png("../submission preparation/graph_panels/panel6/heatmap_binomial_glmm.png", width = 500, height = 500)
pheatmap(mat[,rownames(annotation_df)], 
         main = paste0("Binomial-glmm DEGs \nnot identified by Poisson-glmm (",nrow(mat)," DEGs)"),
         annotation_col = annotation_df, 
         annotation_colors = annotation_colors,
...
```{r eval=FALSE}
png("../submission preparation/graph_panels/panel6/heatmap_MAST_donor.png", width = 500, height = 500)
pheatmap(mat[,rownames(annotation_df)], 
         main = paste0("MAST DEGs \nnot identified by Poisson-glmm (",nrow(mat)," DEGs)"),
         annotation_col = annotation_df, 
         annotation_colors = annotation_colors,
...
### MMvst
```{r}
sort_log2FC = sort(abs(MMvst[[key]]$log2FC[MMvst[[key]]$hits]), index.return = T, decreasing = T)
hitgenes_MMvst = MMvst[[key]]$genes[which(MMvst[[key]]$hits)][sort_log2FC$ix]
hits_notin_glmm = na.omit(hitgenes_MMvst[!hitgenes_MMvst%in%hitgenes])
mat = assays(subgroupsce)$counts[hits_notin_glmm,]

# cell level
pheatmap(mat[,rownames(annotation_df)], 
         main = paste0("MMvst DEGs \nnot identified by Poisson-glmm (",nrow(mat)," DEGs)"),
         annotation_col = annotation_df, 
         annotation_colors = annotation_colors,
         cluster_rows=F, cluster_cols=F, 
         show_colnames = F, show_rownames = F,
...
```{r eval=FALSE}
png("../submission preparation/graph_panels/panel6/heatmap_MMvst.png", width = 500, height = 500)
pheatmap(mat[,rownames(annotation_df)], 
         main = paste0("MMvst DEGs \nnot identified by Poisson-glmm (",nrow(mat)," DEGs)"),
         annotation_col = annotation_df, 
         annotation_colors = annotation_colors,
...
```{r eval=FALSE}
png("../submission preparation/graph_panels/panel6/heatmap_MMpoisson.png", width = 500, height = 500)
pheatmap(mat[,rownames(annotation_df)], 
         main = paste0("MMpoisson DEGs \nnot identified by Poisson-glmm (",nrow(mat)," DEGs)"),
         annotation_col = annotation_df, 
         annotation_colors = annotation_colors,
...
png("../submission preparation/graph_panels/panel6/heatmap_Extra_hits.png", width = 500, height = 500)
pheatmap(mat[,rownames(annotation_df)], 
         main = paste0("Poisson-glmm DEGs \nnot identified by MMpoisson (",nrow(mat)," DEGs)"),
         annotation_col = annotation_df, 
         annotation_colors = annotation_colors,
...
```{r eval=FALSE}
pdf("../submission preparation/graph_panels/panel6/dotplot.pdf", width = 6, height = 5)
dotplot(go_enrich)
pdf("../submission preparation/graph_panels/panel6/cnetplot.pdf", width = 12, height = 6)
cnetplot(go_enrich, categorySize="pvalue", foldChange=gene_list)
dev.off()
```
...
```{r eval=FALSE}
pdf("../submission preparation/graph_panels/panel6/dotplot2.pdf", width = 6, height = 5)
dotplot(kk, 
        showCategory = 10, 
        title = "Enriched Pathways",
        font.size = 8)
